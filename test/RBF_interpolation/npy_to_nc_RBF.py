...
# Interpolate elevation field from coordinates (x,y) to grid (xx, yy) using radial basis function
input_coordinates = np.stack([lon.ravel(), lat.ravel()],-1)
interpolation_points = np.stack([xx.ravel(), yy.ravel()], -1)
interpolation = RBFInterpolator(input_coordinates, elev_list, neighbors=10, kernel='thin_plate_spline')
elev_grid = interpolation(interpolation_points).reshape(xx.shape)
...
ds = nc.Dataset('bathymetry_RBF.nc', 'w', 'NETCDF4') # using netCDF4 for output format
